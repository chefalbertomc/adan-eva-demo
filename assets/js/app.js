  ...
  div.innerHTML = `
    <div class="login-container" style="max-width: 450px; width: 100%; text-align: center;">
      ...
      <div class="text-[10px] text-gray-600 mt-2">
        v19.7 (Emergency Fix + CheckIn Restore)
        <br>
...
function renderHostessDashboard() {
  ...
  const activeVisits = window.db.getActiveVisitsByBranch(STATE.branch.id);
  const waitlist = window.db.getWaitlist(STATE.branch.id);
  const today = new Date().toISOString().split('T')[0];
  const reservations = window.db.getReservations(STATE.branch.id, today);
  ...
  div.innerHTML = `
    ...
    <header class="flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
      <div class="flex items-center gap-4">
        <img src="assets/img/duckos-logo.png" style="height: 40px; width: auto;" alt="Logo">
        <div>
            <h2 class="text-white text-2xl font-bold font-oswald tracking-wider" style="font-family: 'Oswald', sans-serif;">RECEPCIÓN</h2>
            <div class="text-yellow-500 text-xs tracking-widest uppercase">${STATE.branch.name}</div>
        </div>
      </div>
      <button onclick="handleLogout()" class="text-gray-400 hover:text-white text-xs uppercase tracking-widest border border-gray-700 px-4 py-2 rounded-full transition-all hover:border-white">CERRAR SESIÓN</button>
    </header>

    <!-- Tab Content: Check-In -->
    <div id="content-checkin" class="tab-content active">
      <div class="max-w-2xl mx-auto">
         <!-- Check-in Form - PREMIUM DESIGN -->
         <div>
           <div class="card relative overflow-hidden" style="border: 1px solid rgba(255, 210, 0, 0.3); background: linear-gradient(135deg, rgba(20,20,20,1) 0%, rgba(30,30,30,1) 100%);">
              <div class="absolute top-0 left-0 w-1 h-full bg-yellow-500"></div>
              <h3 class="text-3xl mb-8 text-white font-oswald flex items-center gap-3" style="font-family: 'Oswald', sans-serif;">
                <span class="text-yellow-500">📋</span> NUEVO CHECK-IN
              </h3>
              
              <!-- PASO 1: CLIENTE -->
              <div class="mb-8">
                <div class="text-base font-bold text-yellow-500 mb-4 tracking-widest uppercase border-b border-gray-800 pb-2">PASO 1: DATOS DEL CLIENTE</div>
                ...
                <div class="relative mb-6">
                    <input 
                      ...
                      autocomplete="off"
                      class="w-full text-2xl p-4 bg-transparent border-b-2 border-yellow-500/50 text-white placeholder-gray-500 focus:border-yellow-500 focus:outline-none transition-all font-inter"
                      style="font-family: 'Inter', sans-serif; height: 60px;">
                    <div id="search-results" class="hidden absolute top-full left-0 w-full bg-black border border-yellow-500 max-h-60 overflow-auto z-50 mt-1 shadow-2xl"></div>
                </div>
                
                <div class="grid grid-cols-2 gap-6 mb-6">
                    <input id="h-lastname" placeholder="APELLIDO PATERNO" class="text-lg p-4 bg-gray-900/50 border border-gray-700 rounded text-white focus:border-yellow-500 focus:outline-none placeholder-gray-600" onfocus="dismissSearchResults()" style="height: 55px;">
                    <input id="h-lastname2" placeholder="APELLIDO MATERNO" class="text-lg p-4 bg-gray-900/50 border border-gray-700 rounded text-white focus:border-yellow-500 focus:outline-none placeholder-gray-600" onfocus="dismissSearchResults()" style="height: 55px;">
                </div>

                <div class="flex items-center gap-6 bg-gray-900/40 p-3 rounded border border-gray-800">
                    <span class="text-gray-400 text-lg pl-2 font-bold uppercase tracking-wider">PERSONAS:</span>
                    <input id="h-pax" type="number" value="2" class="w-full text-3xl p-3 text-center font-bold bg-transparent text-white focus:outline-none border-none" onfocus="dismissSearchResults()" style="height: 60px;">
                </div>
              </div>

              <!-- PASO 2: MESA Y MESERO -->
              <div id="section-mesa-mesero" class="mb-8">
                <div class="text-base font-bold text-green-500 mb-4 tracking-widest uppercase border-b border-gray-800 pb-2">PASO 2: ASIGNACIÓN</div>
                
                <div class="flex flex-row gap-6">
                  <div class="flex-1">
                    <label class="text-sm text-green-500 font-bold mb-2 block tracking-wider uppercase">MESA #</label>
                    <input id="h-table" type="text" placeholder="Be" 
                           class="w-full text-2xl p-4 bg-transparent border-2 border-green-500/50 rounded text-center text-white focus:border-green-400 focus:outline-none placeholder-gray-600 font-bold" 
                           style="height: 70px;" onfocus="dismissSearchResults()">
                  </div>
                  <div class="flex-1">
                    <label class="text-sm text-green-500 font-bold mb-2 block tracking-wider uppercase">MESERO</label>
                    <select id="h-waiter" 
                            class="w-full text-xl p-4 bg-black border-2 border-green-500/50 rounded text-white focus:border-green-400 focus:outline-none font-bold"
                            style="height: 70px;">
    ...
    `<option value="${w.id}">${w.name}</option>`
              ...
              <!-- PASO 3 (OPCIONAL): LISTA DE ESPERA -->
              <div id="section-waitlist-phone" class="mb-8 hidden">
                 <div class="text-base font-bold text-yellow-500 mb-3 tracking-wider uppercase border-b border-gray-800 pb-2">CONTACTO (LISTA DE ESPERA)</div>
                 <input id="h-phone" type="tel" placeholder="TELÉFONO (10 Digitos)" class="w-full text-2xl p-4 text-center font-bold bg-gray-900 border-2 border-yellow-600 rounded text-white focus:border-yellow-400 focus:outline-none placeholder-gray-600" style="height: 60px;">
              </div>
              
              <!-- BOTONES DE ACCIÓN (TOUCH OPTIMIZED) -->
              <div class="grid grid-cols-1 gap-6 mt-10">
                <button type="button" onclick="doCheckIn()" id="btn-checkin" 
                        class="text-2xl py-6 font-black rounded font-oswald tracking-widest transition-transform active:scale-95 shadow-lg shadow-yellow-500/20" 
                        style="background: linear-gradient(180deg, #FFE066 0%, #FFD700 30%, #CC9900 70%, #AA7700 100%); color: #000; border: none; font-family: 'Oswald', sans-serif; text-shadow: 0 1px 0 rgba(255,255,255,0.4);">
                  ASIGNAR MESA
                </button>
                
                <button type="button" onclick="toggleWaitlistMode()" id="btn-waitlist-toggle" 
                        class="text-xl py-5 font-bold rounded border-2 border-yellow-600 font-oswald tracking-widest text-[#FFD700] hover:bg-yellow-900/40 transition-colors uppercase bg-gray-900 shadow-md" 
                        style="font-family: 'Oswald', sans-serif; letter-spacing: 2px;">
                  ⏱️ AGREGAR A LISTA DE ESPERA
                </button>
                
                <button type="button" onclick="doAddToWaitlist()" id="btn-waitlist-confirm" 
                        class="text-2xl py-6 font-black rounded font-oswald tracking-widest transition-transform active:scale-95 hidden shadow-lg" 
                        style="background: linear-gradient(180deg, #FFE066 0%, #FFD700 30%, #CC9900 70%, #AA7700 100%); color: #000; border: none; font-family: 'Oswald', sans-serif;">
                  CONFIRMAR ESPERA
                </button>
                <button type="button" onclick="cancelWaitlistMode()" id="btn-waitlist-cancel" class="text-lg py-4 font-bold rounded-lg hidden bg-gray-800 text-gray-400 border border-gray-700">
                  ← CANCELAR
                </button>
              </div>
              
              <!-- MENSAJE DE ESTADO -->
              <div id="checkin-status" class="hidden mt-6 p-4 rounded-lg text-center text-xl font-bold"></div>
           </div>
         </div>
    ...
    <!-- Tab Content: GESTIONAR MESAS (CON FILTRO) -->
    <div id="content-tables" class="tab-content hidden">
      <div class="card relative bg-black/95 border border-gray-800 shadow-2xl p-6 rounded-2xl">
        <div class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-yellow-600 to-yellow-300"></div>
        <div class="flex justify-between items-center mb-8 flex-wrap gap-4 border-b border-gray-800 pb-6">
          <h3 class="text-3xl text-white font-oswald flex items-center gap-3">
            <span class="text-yellow-500 text-4xl">🍽️</span> 
            MESAS ACTIVAS <span class="bg-gray-800 text-yellow-500 px-3 py-1 rounded-full text-lg ml-2 border border-gray-700 font-mono">${activeVisits.length}</span>
          </h3>
          <!-- FILTRO POR MESERO -->
          <div class="flex items-center gap-4 bg-gray-900 border border-gray-700 rounded-lg p-2 pr-4">
            <label class="text-base text-gray-400 pl-2 uppercase font-bold tracking-wider">Filtrar por Mesero:</label>
            <select id="filter-waiter" onchange="filterTablesByWaiter()" class="p-3 text-lg bg-black text-white rounded border border-yellow-600 hover:border-yellow-400 focus:outline-none font-bold transition-colors cursor-pointer" style="min-width: 200px;">
    ...
    return `
              <div class="table-card relative bg-zinc-900/90 border-2 border-green-500/30 p-6 rounded-xl shadow-2xl" data-waiter-id="${v.waiterId}" style="background: linear-gradient(145deg, #111 0%, #0a0a0a 100%);">
                ...
                <div class="flex justify-between items-start mb-6 border-b border-gray-800 pb-4">
                    <div>
                        <div class="text-5xl font-black text-white font-oswald tracking-tighter flex items-center gap-2" style="text-shadow: 0 0 15px rgba(74,222,128,0.3);">
                           <span class="text-green-500/80 text-3xl">#</span>${v.table}
                        </div>
                        <div class="text-2xl text-gray-200 font-oswald mt-2 tracking-wide">${customerName}</div>
                        <div class="text-base text-gray-500 mt-2 flex items-center gap-3 font-medium">
                            <span class="bg-gray-800 px-3 py-1 rounded text-xs uppercase font-bold tracking-wider text-gray-300 border border-gray-700">🤵 ${waiterName.split(' ')[0]}</span>
                            <span>👥 ${v.pax} pax</span>
                        </div>
                    </div>
                    <!-- Timer -->
                    <div class="text-right bg-black/60 p-4 rounded-xl border border-gray-800 shadow-inner">
                        <div class="text-4xl font-bold font-mono tracking-tighter" style="color: ${timeColor};">${timeElapsed}</div>
                        <div class="text-[10px] text-gray-500 mt-1 uppercase tracking-widest font-bold">Tiempo Activo</div>
                    </div>
                </div>

                <!-- Motivo y Partido (Inputs grandes) -->
                <div class="mb-6 bg-gray-900/30 p-4 rounded-xl border border-gray-800/50">
                    <label class="text-xs text-green-500 mb-2 block uppercase font-bold tracking-widest px-1">MOTIVO DE VISITA</label>
                    
                    <!-- Select Motivo -->
                    <select onchange="window.updateHostessReason('${v.id}', this)" class="w-full bg-black text-white border-2 border-gray-700/50 rounded-lg p-4 text-xl mb-4 focus:border-green-500 outline-none transition-all appearance-none font-bold" style="height: 60px;">
                        <option value="Comer" ${v.reason === 'Comer' ? 'selected' : ''}>🍽️ Comer</option>
                        <option value="Beber" ${v.reason === 'Beber' ? 'selected' : ''}>🍺 Beber</option>
                        <option value="Partido" ${v.reason === 'Partido' ? 'selected' : ''}>🏈 Ver Partido</option>
                    ...
                    <!-- Bloque Partido (Visible solo si es Partido) -->
                    <div id="hostess-game-select-${v.id}" class="${v.reason === 'Partido' ? '' : 'hidden'} animate-fade-in border-t border-gray-800 pt-5 mt-2">
                         <label class="text-xs text-yellow-500 mb-2 block uppercase font-bold tracking-widest px-1">SELECCIÓN DE PARTIDO</label>
                         <!-- Select Partido -->
                         <select onchange="window.updateHostessGame('${v.id}', this)" class="w-full bg-gray-900 text-yellow-400 border border-yellow-600/50 rounded-lg p-3 text-lg focus:outline-none font-bold mb-4 shadow-[0_0_15px_rgba(234,179,8,0.05)]" style="height: 55px;">
        ...
        return '<option disabled>Equipos no identificados</option>';
                    ...
                    <div class="flex gap-3 mt-6">
                        <button onclick="window.saveVisitInfo('${v.id}')" 
                             class="flex-1 text-black font-black py-4 rounded-lg text-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest hover:brightness-110"
                             style="background: linear-gradient(180deg, #Facc15 0%, #ca8a04 100%); letter-spacing: 1px;">
                           💾 Guardar
                        </button>
                        
                        <button onclick="document.getElementById('management-${v.id}').classList.toggle('hidden')" 
                             class="flex-1 bg-gray-800 border border-gray-600 text-white font-bold py-4 rounded-lg text-lg shadow-lg active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest hover:bg-gray-700">
                           ⚙️ Gestionar
                        </button>
                    </div>
                </div>
